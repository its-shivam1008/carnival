    ...
    <div className='h-screen w-full bg-slate-900 absolute top-0 left-0 flex justify-center items-center bg-opacity-50'>
      <div className="bg-white min-w-[300px] min-h-[100px] flex flex-col justify-center items-center rounded p-4">
        ...
        <div className='flex gap-8'>
          ...
          <button type="button" onClick={() => handleHangup({ongoingCall:ongoingCall ? ongoingCall : undefined, isEmitHangup:true })} className="w-10 h-10 bg-red-500 rounded-full flex items-center justify-center text-white"><MdCall size={24}/></button>
        </div>
      </div>
    </div>
  )
